...
begin
    ...
    copy(c,aux);
    esta_en_la_cola:= False;
    while (not empty_queue(aux)) and (not esta_en_la_cola) do begin
        esta_en_la_cola:= first(aux) = n;
        dequeue(aux);
    end;
end;
...
begin
    posicion_en_cola:= -1;
    if esta_en_la_cola(c,n) then begin
      initialize_queue(aux);
      copy(c,aux);
      contador:= 1;
      while posicion_en_cola = -1 do begin
          if first(aux) = n then
              posicion_en_cola:= contador;
          inc(contador);
          dequeue(aux);
      end;
    end;
...
begin
    ...
    {-- 1º caso para ver si será creciente o decreciente}
    ant:= first(aux);
    dequeue(aux);
    if not empty_queue(aux) then begin
      if ant < first(aux) then
          cola_ordenada:= 1
      else if ant > first(aux) then
          cola_ordenada:= -1;
    {----------------------------------------------------}

      while (not empty_queue(aux)) and (cola_ordenada <> 0) do begin
          ant:= first(aux);
          dequeue(aux);
          if (cola_ordenada = 1) and (ant > first(aux)) then
              cola_ordenada:= 0
          else if (cola_ordenada = -1) and (ant < first(aux)) then
              cola_ordenada:= 0;
      end;
    end
    else
